 ...
 * modalidad — no hace falta volver a pedir la ficha en el paso 6 sólo para
 * los `reqs`. Si vocabulario.js no está, sigue funcionando sin `o.terminos`.
 *
 * OJO con el hash al escribir el id final en `ofertas` (paso 6): usa siempre
 * `ij.idPara(o)` (12 caracteres) en vez de cortarlo a mano. El 16-sep-2026 se
 * descubrió que días distintos habían recortado el hash a longitudes
 * distintas (8 y 10 caracteres) y `dedupe.py` dejó pasar una oferta que ya
 * estaba — ahí se arregló para tolerar prefijos, pero cuanto más consistente
 * sea el id que se escribe, menos hace falta esa tolerancia. */
(function (R) {
  const ij = R.ij = {};
  ij.version = 'infojobs-2026-09-16';
  ...
  /* Id estable para escribir en `ofertas`: 12 caracteres del hash, siempre
   * los mismos, para que un futuro cambio de longitud no vuelva a producir
   * el bug del 16-sep-2026 (ver cabecera del fichero). */
  ij.idPara = o => 'ij-' + o.hash.slice(0, 12);
  ...
  ij.detallar = async (n) => {
    ...
    await R.enTandas(lote, async o => {
      ...
      Object.assign(o, {
        ...
        terminos: _cuentaTerminos(dn),   // null si no hay vocabulario.js cargado
        _dn: dn,
        _desc: desc,
      });
      ij.det.push(o);
  ...
  /* Igual que li.terminos(): usa lo que detallar() ya calculó. */
